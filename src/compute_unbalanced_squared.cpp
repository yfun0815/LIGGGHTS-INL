...
*/
#include <mpi.h>
#include "compute_unbalanced_squared.h"
#include "atom.h"
#include "update.h"
#include "force.h"
#include "domain.h"
...
ComputeUnbalancedSquared::ComputeUnbalancedSquared(LAMMPS *lmp, int &iarg, int narg, char **arg) :
    Compute(lmp, iarg, narg, arg),
    fix_ms(NULL)
{
    if (narg != 3) error->all(FLERR,"Illegal compute unbalanced/squared command");
...
void ComputeUnbalancedSquared::init()
{
...
double ComputeUnbalancedSquared::compute_scalar()
{
    ...
    // for multispheres we need to get the kinetic energy from the multisphere fix
        scalar += fix_ms->extract_unbalancedforce_squared();
    }
    return scalar;
}
