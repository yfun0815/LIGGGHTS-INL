...
void Neighbor::init()
{
  ...
  if (dist_check) {
    if (maxhold == 0) {
      ...
      memory->create(rhold,maxhold,"neigh:rhold"); 
    }
  }
  ...
  if (atom->molecular && atom->nbonds && maxbond == 0) {
    ...
    memory->create(bondlist,maxbond,4,"neigh:bondlist");   
    
    if(atom->n_bondhist)
       memory->create(bondhistlist,maxbond,atom->n_bondhist,"neigh:bondhistlist");
  }
...
/* ----------------------------------------------------------------------
   return center coordinate of bin (ix,iy,iz)
------------------------------------------------------------------------- */

void Neighbor::bin_center(int ix, int iy, int iz, double * center)
{
