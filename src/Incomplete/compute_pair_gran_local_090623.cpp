    ...
    array(NULL)
{
  ...
  overlapratioflag = squaredforceflag = 0; //FEG
    ...
    if(narg > iarg)
        posflag = velflag = idflag = fflag = fnflag = ftflag = torqueflag = torquenflag = torquetflag = histflag = areaflag = deltaflag = heatflag = cpflag = msidflag = overlapratioflag = squaredforceflag = 0; //FEG - added overlapratioflag, squardforceflag
...
void ComputePairGranLocal::init_cpgl(bool requestflag)
{
    ...
    nvalues = posflag*6 + velflag*6 + idflag*3 + fflag*3 + fnflag*3 + ftflag*3 + torqueflag*3 + torquenflag*3 + torquetflag*3 + histflag*dnum + areaflag + deltaflag + heatflag + cpflag*3 + msidflag*2 + overlapratioflag + squaredforceflag; //FEG - added overlapratioflag, squaredforceflag
...
void ComputePairGranLocal::post_force_pp(const LCM::SurfacesIntersectData &sidata, const LCM::ForceData &i_forces, const LCM::ForceData &j_forces)
{
	...
	{ // FEG
	    const double radi = atom->radius[i]; // FEG
	    const double radj = atom->radius[j]; // FEG
	    double del[3]; // FEG
	    vectorSubtract3D(atom->x[i],atom->x[j],del); // FEG
	    array[ipair][n++] = 0.5*(radi+radj-vectorMag3D(del))/std::min(radi,radj); // FEG - 0.5 factor is to make sure overlap ratio is overlap distance (delta) divided by minimum diameter
	} // FEG
    } // FEG
...
void ComputePairGranLocal::post_force_pw(const LCM::SurfacesIntersectData &sidata, const LCM::ForceData &i_forces, const LCM::ForceData &j_forces)
{
    ...
    if(overlapratioflag) // FEG
    { // FEG
      array[ipair][n++] = 0.5*(atom->radius[i]-sqrt(sidata.rsq)) / (atom->radius[i]); // FEG
    } // FEG
    if(squaredforceflag) // FEG
    { // FEG
        array[ipair][n++] = fx*fx + fy*fy + fz*fz; // FEG
